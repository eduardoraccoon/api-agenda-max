...
namespace api_iso_med_pg.Controllers
{
    ...
    public class ClientsController : ControllerBase
    {
        ...
        public ClientsController(IClienteRepository repository, IMapper mapper)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<BaseResponse<IEnumerable<ClientDto>>>> Get()
        {
            try
            {
                var clients = await _repository.GetAllAsync();
                var dtos = _mapper.Map<IEnumerable<ClientDto>>(clients);
                return Ok(new BaseResponse<IEnumerable<ClientDto>>
                {
                    IsSuccess = true,
                    Data = dtos,
            ...
            {
                return StatusCode(500, new BaseResponse<IEnumerable<ClientDto>>
                {
                    IsSuccess = false,
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<BaseResponse<ClientDto>>> Get(int id)
        {
            try
            {
                var client = await _repository.GetByIdAsync(id);
                if (client == null)
                    return NotFound(new BaseResponse<ClientDto>
                    {
                        IsSuccess = false,
                        Message = ReplyMessage.MESSAGE_QUERY_EMPTY
                    });
                var dto = _mapper.Map<ClientDto>(client);
                return Ok(new BaseResponse<ClientDto>
                {
                    IsSuccess = true,
                    Data = dto
                });
            ...
            {
                return StatusCode(500, new BaseResponse<ClientDto>
                {
                    IsSuccess = false,
        ...
        [HttpPost]
        public async Task<ActionResult<BaseResponse<string>>> Post(CreateClientDto dto)
        {
            try
            {
